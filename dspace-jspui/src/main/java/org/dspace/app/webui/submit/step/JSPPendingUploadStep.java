 ...
 * @version $Revision$
 */
public class JSPPendingUploadStep extends JSPStep
{
            ...
            AuthorizeException
    {
        ...
        if (buttonPressed.equalsIgnoreCase(UploadStep.SUBMIT_SKIP_BUTTON) ||
            ...
            log.debug("JSPUI Upload post-processing: " + subInfo.isAddingFulltext());
            log.debug("JSPUI Upload post-processing has pending check: " + subInfo.hasPending());
            String pendingBundle = ConfigurationManager.getProperty("submit.fulltext.bundle.pending");
            String bundleName = ((subInfo.isAddingFulltext() || subInfo.hasPending()) ? pendingBundle : "ORIGINAL");
    ...
    protected void showUploadPage(Context context, HttpServletRequest request,
        ...
        log.debug("Show Upload Page (JSPUI): adding fulltext was " + subInfo.isAddingFulltext());
        log.debug("Show Upload Page (JSPUI): has pending is " + subInfo.hasPending());

        String pendingBundle = ConfigurationManager.getProperty("submit.fulltext.bundle.pending");
        String bundleName = ((subInfo.isAddingFulltext() || subInfo.hasPending()) ? pendingBundle : "ORIGINAL");

        Bundle[] bundles = subInfo.getSubmissionItem().getItem().getBundles(bundleName);
