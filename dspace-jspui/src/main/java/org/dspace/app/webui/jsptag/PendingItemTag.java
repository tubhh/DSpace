 ...
 * @version $Revision$
 */
public class PendingItemTag extends TagSupport {
	...
	/** log4j logger */
	private static Logger log = Logger.getLogger(PendingItemTag.class);

	private static final long serialVersionUID = -3841266490729417240L;

	public PendingItemTag() {
		super();
		getThumbSettings();
	}
	...
	private void listBitstreams() throws IOException {
		...
		try {
			String bundleName = "PENDING";
			String pendingBundle = ConfigurationManager.getProperty("submit.fulltext.bundle.pending");
			if (pendingBundle != null) {
				bundleName = pendingBundle;
			}
			Bundle[] originalBundles = item.getBundles("ORIGINAL");
			Bundle[] pendingBundles = item.getBundles(bundleName);

			Bundle[] bundles;

			if (ConfigurationManager.getBooleanProperty("submit.fulltext.preview_all")) {
				bundles = (Bundle[]) ArrayUtils.addAll(originalBundles, pendingBundles);
			} else {
			...
			if (filesExist) {
				...
				if (bunds.length > 0 && bunds[0] != null) {
