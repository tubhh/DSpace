...
public class DiscoverySearchRequestProcessor implements SearchRequestProcessor {
	...
	public void doOpenSearch(Context context, HttpServletRequest request, HttpServletResponse response)
		...
		Document resultsDoc = OpenSearch.getResultsDoc(format, query, (int) qResults.getTotalSearchResults(),
				qResults.getStart(), qResults.getMaxResults(), container, dsoResults, labelMap);
		try {
	 ...
	 */
	@Override
	public void doAdvancedSearch(Context context, HttpServletRequest request, HttpServletResponse response)
			throws SearchProcessorException, IOException, ServletException {
	 ...
	 */
	@Override
	public void doItemMapSearch(Context context, HttpServletRequest request, HttpServletResponse response)
			throws SearchProcessorException, ServletException, IOException {
		...
		for (DSpaceObject dso : resultDSOs) {
			if (dso != null && dso.getType() == Constants.ITEM) {
				// no authorization check is required as discovery is right
				// aware
				Item item = (Item) dso;
	...
	public void doGlobalSearch(Context context, HttpServletRequest request, HttpServletResponse response)
		...
		if (searchSortConfiguration != null) {
			List<DiscoverySortFieldConfiguration> sortFields = searchSortConfiguration
					.getSortFields();
		...
		request.setAttribute("location", DiscoveryConfiguration.GLOBAL_CONFIGURATIONNAME);
		request.setAttribute("query", query);
		request.setAttribute("queryArgs", queryArgs);
		List<DiscoverySearchFilter> availableFilters = discoveryConfiguration.getSearchFilters();
		request.setAttribute("availableFilters", availableFilters);
		...
		try {
			...
			DiscoveryViewAndHighlightConfiguration discoveryViewAndHighlightConfigurationByName = SearchUtils.getDiscoveryViewAndHighlightConfigurationByName(configurationName);
			request.setAttribute("viewMetadata", discoveryViewAndHighlightConfigurationByName.getViewConfiguration());
			...
			log.error(
					LogManager.getHeader(context, "search",
							"query=" + queryArgs.getQuery() + ",error=" + e.getMessage()), e);
			request.setAttribute("search.error", true);
			request.setAttribute("search.error.message", e.getMessage());
		}

		JSPManager.showJSP(request, response, "/search/global.jsp");
