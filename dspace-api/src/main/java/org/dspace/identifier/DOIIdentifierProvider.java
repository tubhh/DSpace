    ...
    extends IdentifierProvider
{
    ...
    /**
     * The namespaceseparator is a static string DSpace puts into the DOIs its
     * generating to separate several applications that generate DOIs.
     ...
     * This fields contains a string representing the configuration property key.
     */
            ...
            throws SQLException, DOIIdentifierException
    {
        ...
        if (null != doi)
        {
            ...
            if (null != doiRow)
            {
                if (doiRow.isColumnNull("resource_id"))
                {
        ...
        else
        {
            ...
            doi = this.getPrefix() + "/" + getNamespaceSeparator(context, dso) + doiRow.getIntColumn("doi_id");
    ...
    protected String getNamespaceSeparator(Context context, DSpaceObject dso) throws SQLException
    {
        if (dso instanceof Item)
        {
            ...
            if (col != null)
            {
                ...
                if (this.configurationService.getProperty(configkey) != null)
                {
                    return this.configurationService.getProperty(configkey);
                }
            }
        }
        // fallback to the default namespaceseparator
        String separator = this.configurationService.getProperty(CFG_NAMESPACE_SEPARATOR);
        return (separator == null) ? "" : this.configurationService.getProperty(CFG_NAMESPACE_SEPARATOR);
    }

    protected String getNamespaceSeparator(String identifier)
            throws DOIIdentifierException
    {
        String doiPrefix = getPrefix();
        String namespaceseparator = "";
        List<String> separators = getAllNamespaceSeparators();
        for (String separator : separators)
        {
            String identifierPrefix = DOI.SCHEME.concat(doiPrefix).concat(String.valueOf(SLASH)).concat(separator);
            if (StringUtils.startsWithIgnoreCase(identifier, identifierPrefix))
            {
                if (namespaceseparator.length() < separator.length())
                {
    ...
    public static String getDOIOutOfObject(DSpaceObject dso)
        ...
        {
            for (String separator : getAllNamespaceSeparators())
            {
                if (id.value.startsWith(DOI.RESOLVER + String.valueOf(SLASH) + getPrefix() + String.valueOf(SLASH) + separator))
                {
    ...
    protected static List<String> getAllNamespaceSeparators()
    {
        List<String> separators = new ArrayList<>();
        Map<String, String> properties = (new DSpace()).getSingletonService(ConfigurationService.class).getAllProperties();
        for (String key : properties.keySet()){
            if (StringUtils.startsWithIgnoreCase(key, CFG_NAMESPACE_SEPARATOR))
            {
                separators.add(properties.get(key));
            }
        }
        return separators;
    }
