...
<dspace:browselist items="<%= (BrowseItem[])info.getItems() %>" config="crisou.${info[holder.shortName].type}" sortBy="<%= new Integer(info.getSo().getNumber()).toString() %>" order="<%= infoOrder %>" type="<%= info.getType() %>"/>
</div>
</div>
<% } %>
