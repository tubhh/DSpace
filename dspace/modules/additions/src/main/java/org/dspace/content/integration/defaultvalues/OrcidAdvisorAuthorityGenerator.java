...
public class OrcidAdvisorAuthorityGenerator implements EnhancedValuesGenerator
{

    private ApplicationService applicationService;
    private static final Logger log = Logger
            .getLogger(OrcidAdvisorAuthorityGenerator.class);
...
log.debug("Starting to enhance item "+item.getID()+"  (Handle "+item.getHandle()+") on "+schema+"."+element+"."+qualifier+" with value "+value);
        ...
        int num = item.getDC("contributor", "advisor", Item.ANY).length;
        Metadatum[] m = item.getDC("contributor", "advisor", Item.ANY);
        String[] values = new String[num];
        String[] authorities = new String[num];
        int[] confidences = new int[num];
log.debug("Found "+num+" advisor elements");
        for (int idx = 0; idx < num; idx++){
log.debug("Found advisor element "+m[idx].value+" at Item ID "+item.getID()+" (Handle "+item.getHandle()+")");
...
log.debug("Found ORCID "+orcid+" for advisor "+m[idx].value);
