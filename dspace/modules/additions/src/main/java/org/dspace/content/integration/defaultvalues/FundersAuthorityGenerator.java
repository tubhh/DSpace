...
public class FundersAuthorityGenerator implements EnhancedValuesGenerator
{
...
log.debug("project relation has authorityKey "+m[idx].authority);
            ...
            if (ro != null) {
    	        String fundinginformation = ResearcherPageUtils.getStringValue(ro, "funder");
                if (StringUtils.isNotBlank(fundinginformation)) {
log.debug("Found Funding information "+fundinginformation);
                    values[idx] = fundinginformation;
    		    authorities[idx]=m[idx].authority;
    		    confidences[idx]=Choices.CF_ACCEPTED;
    	        }
    	        else {
log.debug("No Funding information set for this project ");
                    values[idx] = "";
    		    authorities[idx]="";
