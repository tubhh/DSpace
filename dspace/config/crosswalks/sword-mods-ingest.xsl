    ...
    <xsl:template match="mods:mods/mods:originInfo">
        ...
        <xsl:if test="mods:place">
            <xsl:for-each select="mods:place/mods:placeTerm">  
                <dim:field mdschema="dcterms" element="publisher" qualifier="place">
                    <xsl:value-of select="."/>
                </dim:field>
            </xsl:for-each>
        </xsl:if>
        ...
        <xsl:if test="mods:part">
            <xsl:for-each select="mods:part/mods:detail">
                <xsl:element name="dim:field">
                    ...
                    <xsl:attribute name="element">container</xsl:attribute>
                    <xsl:attribute name="qualifier">
                        <xsl:value-of select="@type"/>
                    </xsl:attribute>
            ...
            <!--  mods:/relatedItem/part/extent[@unit="pages"]    ====>   dcterms.bibliographicCitation.[pagestart, pageend] -->
            <xsl:if test="mods:part/mods:extent[@unit='pages']">
                <dim:field mdschema="tuhh" element="container" qualifier="startpage">
                    <xsl:value-of select="mods:part/mods:extent/mods:start"/>
                </dim:field>
                <dim:field mdschema="tuhh" element="container" qualifier="endpage">
