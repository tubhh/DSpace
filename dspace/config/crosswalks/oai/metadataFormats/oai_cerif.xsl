...
<xsl:stylesheet
	        ...
	        <oai_cerif:PersonName>
          		...
          		<xsl:choose>
					<xsl:when test="contains(./doc:field[@name='value'], ',')">
						<oai_cerif:FamilyNames><xsl:value-of select="normalize-space(substring-before(., ','))" /></oai_cerif:FamilyNames>
						<oai_cerif:FirstNames><xsl:value-of select="normalize-space(substring-after(., ', '))" /></oai_cerif:FirstNames>
					</xsl:when>
					<xsl:otherwise>
           	...
           	<xsl:for-each select="doc:element[@name='crisrp']/doc:element[@name='orcid']/doc:element/doc:element">
               	<oai_cerif:ORCID>https://orcid.org/<xsl:value-of select="doc:field[@name='value']" /></oai_cerif:ORCID>
           	</xsl:for-each>
	...
	<xsl:template name="publication" match="/">
        ...
        <oai_cerif:Publication id="{$item_prop_id}">
	        ...
	        <xsl:for-each select="doc:element[@name='dc']/doc:element[@name='relation']/doc:element[@name='project']/doc:element/doc:field[@name='value']">
